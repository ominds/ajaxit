(function($) {
    var ajaxItMain={
	  ...
	  if (hash && hash != '#'){
		  ...
		  success:  function (data) {
		      ...
		      // apply new javascript
		      $dataBody.find("script").each(function(){
			  var src = $(this).attr("src");
			  if (!src || $("script[src='"+src+"']").length == 0){
			      $("body").append(this);
			  }
			...
			ajaxItMain.onReady();
		      ...
		      ajaxItMain.initLinks("a:Event(!click)");
		      ajaxItMain.goToHash();
		  }
	      });
	  }
	...
	$(Selector).each(function(){
	    if($(this).attr('href') && $(this).attr('href').search('http://') == -1){
	      if($().live){
		$(this).live('click',function(){ ajaxItMain.goTo($(this).attr('href')); return false;});
	      }else{
		$(this).bind('click',function(){ ajaxItMain.goTo($(this).attr('href')); return false;});
	      }
	    }else{
	      if($(this).attr('href') && $(this).attr('href').match(document.domain)){
		var href=$(this).attr('href').split('/');
		href='/'+href.slice(3).join('/');
		if($().live){
		  $(this).live('click',function(){ajaxItMain.goTo(href); return false;});
		}else{
		  $(this).bind('click',function(){ajaxItMain.goTo(href); return false;});
		}
	      }	
	    }
	});
